...
export default function LogForm() {
  ...
  return (
    <div className="max-w-3xl mx-auto p-6">
      ...
      <h1 className="text-2xl font-semibold text-text-strong mb-6 flex items-center gap-3">
        <i className={`fa-solid ${isEdit ? "fa-pen" : "fa-plus"} text-text-muted`}></i>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <label className="flex flex-col gap-1">
            <span className="text-sm text-text-strong">Activity type *</span>
            <select
          ...
          <label className="flex flex-col gap-1">
            <span className="text-sm text-text-strong">Outcome *</span>
            <select
              value={form.outcome}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <label className="flex flex-col gap-1">
            <span className="text-sm text-text-strong">Target</span>
            <input
              type="text"
              value={form.target}
              onChange={(e) => setField("target", e.target.value)}
              placeholder="e.g. 10.0.0.5"
              className="bg-bg-canvas border border-border-default rounded px-3 py-2 text-text-strong focus:outline-none focus:border-primary font-mono text-sm"
            />
          </label>
          <label className="flex flex-col gap-1">
            <span className="text-sm text-text-strong">Tool used</span>
            <input
              type="text"
              value={form.toolUsed}
        ...
        <label className="flex flex-col gap-1">
          ...
          <textarea
            ...
            required
            placeholder="What did you do? (markdown supported in Slice 9)"
            className="bg-bg-canvas border border-border-default rounded px-3 py-2 text-text-strong focus:outline-none focus:border-primary min-h-[100px]"
          />
        </label>

        <label className="flex flex-col gap-1">
          <span className="text-sm text-text-strong">Result *</span>
          <textarea
            value={form.result}
        ...
        <div className="flex flex-col gap-1">
          <span className="text-sm text-text-strong">Tags</span>
          <div className="flex gap-2">
            <input
              ...
              className="bg-bg-canvas border border-border-default rounded px-3 py-2 text-text-strong focus:outline-none focus:border-primary flex-1"
            />
            <button
              type="button"
              onClick={addTag}
              className="border border-border-default rounded px-3 py-2 text-sm text-text-strong hover:bg-bg-inset"
            >
          ...
          {form.tags.length > 0 && (
            <div className="flex flex-wrap gap-1 mt-2">
              {form.tags.map((tag) => (
                <span
                  key={tag}
                  className="text-xs bg-bg-inset text-text-body px-2 py-1 rounded font-mono flex items-center gap-1"
                >
                  {tag}
                  <button
                    type="button"
        ...
        <div className="flex flex-col gap-1">
          <span className="text-sm text-text-strong">Code block (optional)</span>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-2">
            <input
              ...
              maxLength={40}
              className="bg-bg-canvas border border-border-default rounded px-3 py-2 text-text-strong focus:outline-none focus:border-primary font-mono text-sm md:col-span-1"
            />
            <textarea
              value={form.codeBlock}
              onChange={(e) => setField("codeBlock", e.target.value)}
              placeholder="Paste command output, code, etc."
              className="bg-bg-canvas border border-border-default rounded px-3 py-2 text-text-strong focus:outline-none focus:border-primary font-mono text-sm min-h-[80px] md:col-span-3"
            />
          </div>
        </div>
        ...
        <div className="flex gap-2 mt-2">
          ...
          <button
            type="button"
            onClick={() => navigate(-1)}
            className="border border-border-default rounded px-4 py-2 text-sm text-text-strong hover:bg-bg-inset"
          >
            Cancel
          </button>
        </div>
      ...
      <p className="text-text-muted text-sm mt-6 flex items-center gap-2">
        ...
        New logs save as Draft. Submit for review comes in Slice 7.
      </p>
    </div>
  );
}
